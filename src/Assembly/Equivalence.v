...
Global Instance show_lines_EquivalenceCheckingError : ShowLines EquivalenceCheckingError
  := fun parens err => match err with
                       ...
                       | Not_enough_input_dag_nodes t
                         => ["Internal error: Not enough input dag nodes to allocate for type " ++ show false t ++ "."]%string
                       | Invalid_argument_type t
                         => ["Invalid type for argument: " ++ show false t]%string
                       ...
                       | Invalid_higher_order_application var s d f x
                         => let __ := @Compilers.ToString.PHOAS.expr.partially_show_expr in
                            ["Invalid higher-order application node with argument of type " ++ show false s ++ ":"
                             ; "Function expression: " ++ show false f
                             ; "Argument expression: " ++ show false x]%string
                       | Invalid_higher_order_let var s x
                         => let __ := @Compilers.ToString.PHOAS.expr.partially_show_expr in
                            ["Invalid higher-order let-in node with argument of type " ++ show false s ++ ":"
                             ; "Let-bound expression: " ++ show false x]%string
                       | Unhandled_identifier t idc
                         => ["Identifier not yet handled by symbolic evaluation: " ++ show false idc ++ "."]%string
                       end%list.
